...
/// Dimensions of the buffer passed to `libretro_bridge_video`. When set, a dynamic-resolution frame
/// larger than this in either dimension is clamped (never copied past the buffer, and the row stride
/// stays equal to the reported width so the frame doesn't scramble). 0 = unbounded.
