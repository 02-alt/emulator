...
/// Lock-free single-producer/single-consumer ring of interleaved Int16 samples. The emulation
/// thread writes; the real-time audio thread reads. Counts are in Int16 *slots* (a stereo
/// sample-frame is 2 slots). Capacity is rounded up to a power of two.
///
/// This mirrors the macOS app's ring buffer — kept as its own copy here because the macOS one lives
/// in the AppKit-only `emu-window` target.
